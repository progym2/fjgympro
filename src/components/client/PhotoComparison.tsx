...
import { 
  ArrowLeftRight, Calendar, Scale, Percent, ChevronDown,
...
const PhotoComparison: React.FC<PhotoComparisonProps> = ({ photos }) => {
  ...
  const sortedPhotos = useMemo(() => 
    [...photos].sort((a, b) => new Date(a.photo_date).getTime() - new Date(b.photo_date).getTime()),
    [photos]
  );

  // Auto-select first and last photo if not selected
  React.useEffect(() => {
    if (sortedPhotos.length >= 2 && !beforeId && !afterId) {
